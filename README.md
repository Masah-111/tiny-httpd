...
flowchart TD
    ...
    B -->|-DUSE_NONBLOCK| N[非ブロッキング状態機械<br/>単一スレッド・一切止まらない]
    C --> E[リクエスト行とヘッダを読む]
    D --> E
    N --> E
    E --> F{厳格な検証}
...
| 機能 | 説明 |
|---|---|
| **同時接続** | 3 方式。標準は接続ごとにスレッド、Linux 向けに **epoll + スレッドプール**（C10K）と、socket を一切ブロックさせない **非ブロッキング状態機械**（単一スレッド・Slowloris 完全耐性）|
...
待機中の接続はスレッドを消費しないので**アイドルな** C10K は捌けるが、**敵対的な**低速接続には弱い。

この版では socket に一切ブロックさせない。

- **読み**: リクエストが揃うまで接続ごとのバッファに貯め、揃った時点で初めて処理する
- **処理**: 応答の書き出しはすべて送信バッファへ積むだけ（socket に触れない）
- **書き**: epoll が「書ける」と言うたびに、バッファ → ファイル本文（`sendfile`）の順で少しずつ送る
- 送れない／届いていないときは即座に epoll へ戻るので、**スレッドは 1 本も止まらない**

そのためワーカープールすら不要で、**単一スレッド**で動く。非ブロッキングでは `SO_RCVTIMEO` が
効かないので、接続ごとに期限を持たせて毎秒掃除している（実測どおり 10 秒で回収される）。
...
ヘッダを途中まで送って沈黙する接続を大量に張った状態で、別の正常なリクエストが通るかを比較した。

| 低速接続 | epoll + スレッドプール | **非ブロッキング状態機械** |
|---|---|---|
| 100 本 | **応答なし**（5 秒でタイムアウト） / 13 スレッド | **200 OK (15 ms)** / **1 スレッド** |
| 500 本 | **応答なし**（5 秒でタイムアウト） / 13 スレッド | **200 OK (26 ms)** / **1 スレッド** |

プール版は**わずか 100 本で完全にサービス停止**する（12 のワーカーが全員 `recv` で止まるため）。
非ブロッキング版は 500 本抱えても単一スレッドで平然と応答する。
「アイドルな 1 万接続を捌ける」と「**敵対的な**低速接続を捌ける」は別物だ、という違いがここに出る。

> 副産物として **HTTP パイプライン**にも対応した。受信バッファに次のリクエストが先読みされていれば、
> 応答を送り切った直後にそのまま処理する。
...
### 現状の既知の割り切り（正直に）
- epoll + プール版のワーカーは「読み取り可能」通知を受けてから**ブロッキングで**読むため、
  極端に遅い相手にワーカーを占有される（実測で 100 本の Slowloris により停止）。
  これを解消したいときは `-DUSE_NONBLOCK` の非ブロッキング版を使う。
- 非ブロッキング版は単一スレッドなので、CPU を食う処理を並列化しない（静的配信では
  I/O 待ちが支配的なので実用上は問題にならないが、設計上の性質として明記しておく）。
- 非ブロッキング版はリクエスト全体（本文含む、上限 1MB）を一度メモリに貯めてから処理する。
  ストリーミング処理ではない。
